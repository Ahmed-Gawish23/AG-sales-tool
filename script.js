document.getElementById("fileInput").addEventListener("change", async (event) => {
  ...
  // البحث عن الصف الأول الصالح
  let jsonData = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: "" });
  let validRowIndex = jsonData.findIndex(row => row.some(cell => cell)); // إيجاد أول صف يحتوي على بيانات
  if (validRowIndex === -1) {
    alert("No valid data found in the file");
    return;
  }
  
  // استخراج البيانات من الصفوف الصالحة فقط
  jsonData = jsonData.slice(validRowIndex);
  const headers = jsonData.shift(); // استخراج أسماء الأعمدة
